...
function renderPage(activeId: string) {
  switch (activeId) {
    ...
    case "assessment-info": return <AssessmentInfo />;
    case "target-profile": return <DefineTargetProfile />;
    case "planning": return <AssessmentPlanning />;
    case "work-products": return <UploadWorkProducts />;
    case "perform": return <PerformAssessment />;
    ...
    <aside className="flex flex-col h-full bg-sidebar w-full">
      ...
      <div className="px-5 pt-6 pb-5 border-b border-sidebar-border">
        <div className="flex items-center gap-2.5">
          <div className="w-8 h-8 rounded-lg spice-gradient flex items-center justify-center shrink-0">
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          <div className="relative w-[260px] h-full">
            <Sidebar active={active} onSelect={setActive} onClose={() => setMobileOpen(false)} />
          </div>
        </div>
      )}
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header className="md:hidden flex items-center gap-3 px-4 py-3 border-b border-border bg-background shrink-0">
          ...
          <div className="flex items-center gap-2">
            <div className="w-6 h-6 rounded spice-gradient flex items-center justify-center">
              <Flame className="h-3 w-3 text-white" />
            </div>
            <span className="font-bold font-heading text-foreground">SpiceUp</span>
          </div>
        </header>
